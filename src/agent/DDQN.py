...
class DDQNAgent():
    ...
    def __init__(self, env, model, target_model, lr, buffer_sz, epsilon, epsilon_decay,
        ...
        self.strategy = EpsilonGreedy(
            self.epsilon, self.min_epsilon, self.epsilon_decay)
    ...
    def train(self, epochs, batch_sz):
        ...
        for epoch in range(epochs):
            ...
            while True:
                ...
                self.memory.store(self.experience(
                ...
                if self.memory.sample_possible(batch_sz):
                    ...
                    # transforms actions tensor into tensor with lists for indexing
                    index = actions.unsqueeze(-1)
                    current_q_values = self.model(states).gather(
                        dim=1, index=index).squeeze()  # squeeze to remove 1 axis

                    # DDQN
                    max_next_q_values_model_indices = self.model(
                        next_states).argmax(1).detach()
                    index_ddqn = max_next_q_values_model_indices.unsqueeze(-1)
                    # Gather Q-Values of target_model for corresponding actions
                    next_q_values_from_target_of_model_indices = self.target_model(next_states).gather(
                        dim=1, index=index_ddqn).squeeze()  # squeeze to remove 1 axis
                    # Update target Q_values with Q-values of target_model based on max Q-values of model
                    target_q_values = (
                        next_q_values_from_target_of_model_indices*self.gamma)+rewards*(1-dones)
    ...
    def _select_action(self, state):
        ...
        """
        self.exploration_rate = self.strategy.get_exploration_rate(
            self.current_step)
        self.current_step += 1
