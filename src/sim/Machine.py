...
class Machine(CoreObject):
    ...
    def __init__(self, id, system, machine_type, output_buffer_capacity):
        ...
        if self.system.degradation_on:
            self.repair_durations = self.production_system.machine_types[
                self.machine_type]['repair_durations']
            self.degradation = self._generate_degradation_matrix(
                self.production_system.machine_types[self.machine_type]['degradation_rate'])
        ...
        for product_type in self.production_system.product_types:
            ...
            if first_task in self.tasks:
                ...
                self.logger.debug('{} can start products'.format(
                    self.id), extra={"simtime": self.sim_env.now})
                break
    ...
    def find_minimum_due_date(self, store):
        ...
        self.logger.debug('{} found minimum due date: {}'.format(
            self.id, min_due_date), extra={'simtime': self.sim_env.now})
        return min_due_date
    ...
    def working(self):
        ...
        while True:
            self.logger.debug("{} Machine.working() started, status: {}".format(
                self.id, self.status), extra={"simtime": self.sim_env.now})
            try:
                ...
                if self.status in [None, 'working', 'waiting', 'repair_finished']:
                    ...
                    if self.production_state == 'processing_part':
                        ...
                        while self.remaining_process_time:
                            self.logger.debug('{} working on product of type {} and due_date {}, time left : {}'.format(
                                self.id, self.product.product_type, self.product.due_date, self.remaining_process_time), extra={'simtime': self.sim_env.now})
                        ...
                        self.logger.debug('{} finished working on product of type {} and due_date {}'.format(
                            self.id, self.product.product_type, self.product.due_date), extra={'simtime': self.sim_env.now})
                    ...
                    if self.production_state == 'putting_product_in_output_buffer':
                        ...
                        self.logger.debug('{} finished putting part in the production_store'.format(
                            self.id), extra={'simtime': self.sim_env.now})
                ...
                self.logger.debug('{} Interrupted while status {} and with cause {}'.format(
                    self.id, self.status, interrupt.cause), extra={'simtime': self.sim_env.now})
                if interrupt.cause == 'from_degrade':
                    self.status = 'failed'
                    self.logger.debug('{} set status to failed'.format(
                        self.id), extra={'simtime': self.sim_env.now})
                elif interrupt.cause == 'from_clock':
                    self.status = 'weekend'
                    self.logger.debug('{} set status to weekend'.format(
                        self.id), extra={'simtime': self.sim_env.now})
                elif interrupt.cause == 'from_scheduler':
                    self.status = 'scheduled_maintenance'
                    self.logger.debug('{} set status to scheduled_maintenance'.format(
                        self.id), extra={'simtime': self.sim_env.now})

    def maintain(self):
        """ Machine gets maintained. Duration is based on repair_type """

        self.logger.debug("{} Machine.maintain() started".format(
            self.id), extra={"simtime": self.sim_env.now})
        ...
        for i in range(self.time_to_repair):
            if i < self.time_to_repair - 1:
                try:
                    self.logger.debug('{} is maintaining, time left: {}'.format(
                        self.id, self.time_to_repair - i), extra={'simtime': self.sim_env.now})
                    yield self.sim_env.timeout(1)
                ...
                try:
                    self.logger.debug('{} is maintaining, shorter timeout, time left: {}'.format(
                        self.id, self.time_to_repair - i), extra={'simtime': self.sim_env.now})
                    yield self.sim_env.timeout(1-self.epsilon)
                except simpy.Interrupt:
                    # ignore interruptions, repair time is fixed
        ...
        if not self.weekly_schedule.is_it_worktime():
            self.logger.debug(
                "{} maintenace finished -> weekend".format(self.id), extra={"simtime": self.sim_env.now})
            self.status = 'weekend'
        else:
            self.logger.debug(
                "{} maintenace finished -> working".format(self.id), extra={"simtime": self.sim_env.now})
            self.status = 'repair_finished'
        self.logger.debug("{} Machine.maintain() completed".format(
            self.id), extra={"simtime": self.sim_env.now})
        yield self.sim_env.timeout(self.epsilon)
            ...
            try:
                ...
                if self.status in [None, 'working']:
                    ...
                    if ((self.health == self.failed_state) and (not self.failed)):
                        ...
                        self.logger.debug("{} Worn out. Product is {}".format(
                            self.id, self.product), extra={"simtime": self.sim_env.now})
                    ...
                    self.logger.debug("{} Machine.degrade() completed".format(
                        self.id), extra={"simtime": self.sim_env.now})
                    yield self.sim_env.timeout(1)
                ...
                self.logger.debug("{} Degradation interrupted by {}".format(
                    self.id, interrupt.cause), extra={"simtime": self.sim_env.now})
