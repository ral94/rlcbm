...
class Order(CoreObject):
    ...
    def __init__(self, products_to_order, due_date, system, order_date=None):
        ...
        for product_type in self.products_to_order:
            assert product_type in self.system.production_system.product_types, 'Tried to order a product of unsupported type {}, please define in ProductionSystem.'.format(
                product_type)
            for _ in range(self.products_to_order[product_type]):
                product = Product(
                    product_type=product_type, production_system=self.production_system, order=self, due_date=self.due_date)
