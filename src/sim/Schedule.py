...
class Schedule:
    ...
    def __init__(self, sim_env, step_duration, work_start_mon, work_end_sat, weekend_on=True):
        ...
        self.steps_per_weekend = self.steps_per_day + \
            ((24 - self.work_end_sat) + (self.work_start_mon)) / step_duration

        self.logger = logging.getLogger("factory_sim")

        self.logger.debug("Schedule successfully created.",
                          extra={"simtime": sim_env.now})
        self.log_time()
    ...
    def log_time(self):
        ...
        # +6 because step 0 is set to monday 6 o'clock
        current_hour = self.work_start_mon + \
            int(current_step * self.step_duration)
        current_minute = int(
            ((current_step * self.step_duration) - int(current_step * self.step_duration)) * 60)
    ...
    def log_time_new_week(self):
        ...
        """
        self.logger.debug("next Week starts in: {}h".format(
            self.get_time_new_week()), extra={"simtime": self.sim_env.now})
